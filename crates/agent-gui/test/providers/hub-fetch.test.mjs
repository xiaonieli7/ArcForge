...
test("prepareUpstreamProxyRequest 保留路径与查询串并携带三个反代头", async () => {
  ...
  assert.equal(prepared.headers["x-arcforge-upstream-origin"], "https://clawhub.ai");
  assert.equal(prepared.headers["x-arcforge-proxy-token"], "test-proxy-token");
  assert.equal(prepared.headers["x-arcforge-use-system-proxy"], "1");
});
...
test("hubFetch 桌面端改写请求地址并合并调用方 headers", async () => {
  ...
  assert.equal(
    headers.get("x-arcforge-upstream-origin"),
    "https://registry.modelcontextprotocol.io",
  );
  assert.equal(headers.get("x-arcforge-proxy-token"), "test-proxy-token");
  assert.equal(headers.get("x-arcforge-use-system-proxy"), "1");
});
...
test("hubFetch 在 Gateway WebUI 运行时直连、不改写地址不加反代头", async () => {
  ...
  const headers = new Headers(calls[0].init.headers);
  assert.equal(headers.get("accept"), "application/json");
  assert.equal(headers.get("x-arcforge-upstream-origin"), null);
  assert.equal(headers.get("x-arcforge-use-system-proxy"), null);
});
