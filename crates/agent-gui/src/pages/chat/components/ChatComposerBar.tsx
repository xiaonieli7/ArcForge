...
export const ChatComposerBar = memo(function ChatComposerBar(props: {
  ...
  const openMentionPicker = useCallback(
    (trigger: "file" | "skill") => {
      ...
      window.requestAnimationFrame(() => composerRef.current?.openMentionPicker(trigger));
    },
    [composerRef],
  );
            ...
            <div className="flex min-w-0 flex-1 items-center gap-1">
              <DropdownMenu open={addMenuOpen} onOpenChange={setAddMenuOpen}>
                <DropdownMenuTrigger
                  ...
                  className={cn(
                    "composer-add-trigger relative inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full px-2.5 text-xs font-medium text-muted-foreground outline-hidden transition-colors hover:bg-muted/60 hover:text-foreground focus-visible:bg-muted/60 focus-visible:text-foreground disabled:pointer-events-none disabled:opacity-40",
                    addMenuOpen && "bg-muted/70 text-foreground",
                    pendingUploadedFiles.length > 0 && "text-sky-600 dark:text-sky-300",
                  )}
                >
                  <Plus
                    className={cn(
                  ...
                  />
                  <span>{t("chat.composer.add")}</span>
                  {pendingUploadedFiles.length > 0 ? (
                    <span className="ml-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-sky-500 px-1 text-[9px] font-semibold leading-none text-white dark:bg-sky-400 dark:text-slate-900">
                ...
                <DropdownMenuContent
                  ...
                  sideOffset={8}
                  collisionPadding={12}
                  className="composer-add-menu w-64 rounded-2xl border-border/70 bg-popover/95 p-1.5 shadow-[0_18px_55px_-18px_rgba(15,23,42,0.38)] outline-hidden backdrop-blur-2xl dark:border-white/[0.10] dark:bg-zinc-900/95"
                >
                  <DropdownMenuItem
                  ...
                  <DropdownMenuItem
                    ...
                    className="gap-3 rounded-xl px-2.5 py-2.5"
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-300">
                      <FileText className="h-4 w-4" />
                    </span>
                    <span className="min-w-0 flex-1 truncate">
                      {t("chat.composer.referenceFiles")}
                    </span>
                    <kbd className="rounded-md border border-border/60 bg-muted/50 px-1.5 py-0.5 font-sans text-[10px] text-muted-foreground">
                      @
                    </kbd>
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    disabled={isInputDisabled || enabledSkills.length === 0}
                    onSelect={() => openMentionPicker("skill")}
                    className="gap-3 rounded-xl px-2.5 py-2.5"
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-violet-500/10 text-violet-600 dark:text-violet-300">
                      <Sparkles className="h-4 w-4" />
                    </span>
                    <span className="min-w-0 flex-1 truncate">
                      {t("chat.composer.referenceSkills")}
                    </span>
                    <kbd className="rounded-md border border-border/60 bg-muted/50 px-1.5 py-0.5 font-sans text-[10px] text-muted-foreground">
                      /
                    </kbd>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate">{t("chat.composer.webSearch")}</span>
                    {chatRuntimeControls.nativeWebSearchEnabled ? (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 truncate">{t("chat.composer.thinking")}</span>
                    {chatRuntimeControls.thinkingEnabled && thinkingSupported ? (
                  ...
                  <GitBranchSelector
                    ...
                    onRequestParentClose={() => setAddMenuOpen(false)}
                  />
                </DropdownMenuContent>
              </DropdownMenu>
              ...
              <ChatModelPicker
                ...
                onSelectReasoning={(reasoning) =>
                  onChatRuntimeControlsChange({ reasoning, thinkingEnabled: true })
                }
              />
            </div>
