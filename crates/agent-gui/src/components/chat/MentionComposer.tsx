...
export interface MentionComposerHandle {
  ...
  openMentionPicker: (trigger: "file" | "skill") => void;
  ...
  forwardRef<MentionComposerHandle, MentionComposerProps>(function MentionComposer(
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        openMentionPicker: (trigger: "file" | "skill") => {
          const el = editorRef.current;
          if (!el || disabled || (trigger === "skill" && enabledSkills.length === 0)) return;

          finishTypewriter();
          resetPromptHistoryRecall();
          focusEditorAtSavedSelection();

          let position = selectionTextPosition(el);
          if (!position) {
            placeCaretAtEditorEnd();
            position = selectionTextPosition(el);
          }

          let needsBoundary = false;
          if (position) {
            const before = (position.textNode.textContent || "").slice(0, position.offset);
            if (before) {
              needsBoundary = !isMentionBoundaryChar(before[before.length - 1]);
            } else {
              const previous = prevLeaf(position.textNode, el);
              if (previous?.nodeType === Node.TEXT_NODE) {
                const previousText = previous.textContent || "";
                needsBoundary =
                  previousText.length > 0 &&
                  !isMentionBoundaryChar(previousText[previousText.length - 1]);
              }
            }
          }
