...
状态：Planned；[机器化合同评审](G1_CONTRACT_REVIEW_V1.md)已达到 `G1 contract review candidate`，但仍须在 G0 产品证据达到最低门并完成启动决定后才能执行
...
起始输入为 [Contract V0](../../contracts/v0/README.md) 的封闭 Schema、Hash Registry、正负例与黄金向量。现有 JavaScript/Python 校验只构成评审基线；WP0 必须增加 Rust/独立第二实现、Fuzz 和 replay 证据后才可讨论冻结。

首个冻结阻断项是 [BC-09](BROKER_CONTRACT_FREEZE_CHECKLIST_V0.md)：为每个 Capability/Operation 固定输入、Preview、Receipt Schema 和 postcondition。`create | replace | delete | DataEgress` 不能共享一个会接受矛盾 after-state 的泛化 `Applied` 规则。
...
停止条件：授权后 InvocationSpec 仍可修改；无法区分 Event Store 事实与 Runtime 观察；或改写 operation 后仍能以矛盾资源后置状态通过 `Applied`。
