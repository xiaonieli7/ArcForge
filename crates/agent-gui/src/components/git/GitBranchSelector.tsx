import { type ElementType, useCallback, useEffect, useId, useMemo, useRef, useState } from "react";
...
export function GitBranchSelector(props: {
  ...
  const closeAllMenus = useCallback(() => {
    ...
    onRequestParentClose?.();
  }, [handleMenuOpenChange, onRequestParentClose]);
  ...
  }, [closeAllMenus, draftBranch, gitClient, runBranchMutation, workdir]);
  ...
  const SelectorRoot: ElementType = presentation === "submenu" ? DropdownMenuSub : DropdownMenu;
  const SelectorTrigger: ElementType =
    presentation === "submenu" ? DropdownMenuSubTrigger : DropdownMenuTrigger;
  const SelectorContent: ElementType =
    presentation === "submenu" ? DropdownMenuSubContent : DropdownMenuContent;

  return (
    <>
      <SelectorRoot open={menuOpen} onOpenChange={handleMenuOpenChange}>
        <SelectorTrigger
          {...(presentation === "submenu" ? { clickToggle: true } : {})}
          disabled={disabled || !gitClient || !workdir.trim()}
          className={cn(
            presentation === "submenu"
              ? "w-full gap-3 rounded-xl px-2.5 py-2.5 text-sm"
        ...
        >
          {presentation === "submenu" ? (
            <>
              ...
              <span className="min-w-0 flex-1 text-left">Git</span>
        ...
        <SelectorContent
          ...
          side={presentation === "submenu" ? "right" : "top"}
          align={presentation === "submenu" ? "end" : "start"}
          collisionPadding={12}
        >
